...
import factory.MonochromaticSequenceCreator;
import factory.MultichromaticSequenceCreator;
...
public class PNGFilterLauncher {
  ...
  static void initializeFilters() {
    ...
    filters.put("--rain-arti",
    		new MultichromaticSequenceCreator(new Rainbow()).addFilter(new Artifacter()).build()
    );
    filters.put("--mono-arti", new MonochromaticSequenceCreator(new Artifacter()).build()
    );
    filters.put("--rgb-mono", new MonochromaticSequenceCreator(new RGBShifter()).build());
